...
async function loadWatch(animeId, ep = 1) {
  ...
  page.innerHTML = `
    <div style="background:#000;padding:0">
      <div class="player-wrap" id="player-wrap" style="border-radius:0;max-width:100%">
        <video id="main-video" class="player-video" poster="${poster}" playsinline></video>
        <div class="speed-indicator" id="speed-indicator">2x Kecepatan</div>
        <div class="seek-indicator seek-left" id="seek-left">-10s</div>
        <div class="seek-indicator seek-right" id="seek-right">+10s</div>
        <div class="player-overlay" id="player-overlay">
          <button class="player-btn" onclick="playerSkip(-10)" style="background:rgba(0,0,0,0.5);border-radius:50%;padding:10px">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M12 5V1L7 6l5 5V7c3.3 0 6 2.7 6 6s-2.7 6-6 6-6-2.7-6-6H4c0 4.4 3.6 8 8 8s8-3.6 8-8-3.6-8-8-8z"/><text x="9" y="15" font-size="6" fill="white">10</text></svg>
          </button>
          <button class="player-btn" id="play-pause-btn" onclick="togglePlay()" style="background:rgba(255,255,255,0.15);border-radius:50%;padding:14px">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="white" id="play-icon"><polygon points="5,3 19,12 5,21"/></svg>
          </button>
          <button class="player-btn" onclick="playerSkip(10)" style="background:rgba(0,0,0,0.5);border-radius:50%;padding:10px">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M12 5V1l5 5-5 5V7c-3.3 0-6 2.7-6 6s2.7 6 6 6 6-2.7 6-6h2c0 4.4-3.6 8-8 8s-8-3.6-8-8 3.6-8 8-8z"/><text x="9" y="15" font-size="6" fill="white">10</text></svg>
          </button>
        </div>
        <div class="player-controls" id="player-controls">
          <div class="player-progress" id="progress-bar" onclick="seekVideo(event)">
            <div class="player-progress-fill" id="progress-fill" style="width:0%">
              <div class="player-progress-thumb"></div>
            </div>
          </div>
          <div class="player-btns">
            <button class="player-btn" onclick="togglePlay()" title="Play/Pause">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="white" id="ctrl-play-icon"><polygon points="5,3 19,12 5,21"/></svg>
            </button>
            <button class="player-btn" onclick="playerSkip(-10)" title="-10s">
            ...
            <button class="player-btn" onclick="playerSkip(10)" title="+10s">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><polyline points="23,4 23,10 17,10"/><path d="M20.49 15a9 9 0 1 1-.49-4.95"/></svg>
            </button>
            <button class="player-btn" onclick="prevEpisode(${animeId},${ep})" title="Episode Sebelumnya" ${ep <= 1 ? 'style="opacity:0.3"' : ''}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><polygon points="19,20 9,12 19,4"/><line x1="5" y1="19" x2="5" y2="5" stroke="white" stroke-width="2"/></svg>
            </button>
            <button class="player-btn" onclick="nextEpisode(${animeId},${ep},${totalEps})" title="Episode Berikutnya">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><polygon points="5,4 15,12 5,20"/><line x1="19" y1="5" x2="19" y2="19" stroke="white" stroke-width="2"/></svg>
            </button>
            <div style="flex:1"></div>
            <span class="player-time" id="player-time">0:00 / 0:00</span>
            <select id="speed-select" onchange="setSpeed(this.value)" style="background:rgba(0,0,0,0.7);color:white;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:3px 6px;font-size:0.78rem;cursor:pointer">
            ...
            <select id="sub-toggle" onchange="toggleSubtitle(this.value)" style="background:rgba(0,0,0,0.7);color:white;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:3px 6px;font-size:0.78rem;cursor:pointer">
              <option value="id">Sub ID</option><option value="en">Sub EN</option><option value="off">Matikan</option>
            </select>
            <button class="player-btn" onclick="toggleFullscreen()" title="Fullscreen">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"/></svg>
            </button>
          </div>
        </div>
      </div>
    ...
    <div style="padding:20px 16px;max-width:900px;margin:0 auto">
      ...
      <div class="glass-card" style="padding:16px;margin-bottom:20px">
        <div style="font-size:0.82rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Sumber Streaming</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button class="filter-chip active" onclick="selectServer(this,'demo')">Server Demo</button>
          <button class="filter-chip" onclick="selectServer(this,'backup1')">Backup 1</button>
          <button class="filter-chip" onclick="selectServer(this,'backup2')">Backup 2</button>
        </div>
        <p style="font-size:0.78rem;color:var(--muted);margin-top:12px">
          ...
          Streaming menggunakan provider eksternal. Kualitas bergantung pada koneksi internet Anda.
        </p>
      </div>
    </div>`;

  initPlayer(animeId, ep, anime);
  saveWatchHistory(animeId, ep, anime);
  setupPlayerGestures();
}

function initPlayer(animeId, ep, anime) {
  const video = document.getElementById('main-video');
  if (!video) return;
  video.playbackRate = App.playerSpeed;
  const speedSel = document.getElementById('speed-select');
  if (speedSel) speedSel.value = App.playerSpeed;

  // Restore position
  const hist = App.watchHistory.find(h => h.id == animeId && h.ep == ep);
  if (hist?.position) {
    video.addEventListener('loadedmetadata', () => { video.currentTime = hist.position; }, {once: true});
  }
  ...
  video.addEventListener('timeupdate', () => {
    ...
    if (Math.floor(video.currentTime) % 10 === 0) {
      ...
      updateWatchProgress(animeId, ep, video.currentTime, progress);
    }
  });
  ...
  video.addEventListener('ended', () => {
    const totalEps = anime?.episodes;
    if (totalEps && ep < totalEps) setTimeout(() => nextEpisode(animeId, ep, totalEps), 3000);
  });
  ...
  if (pw) {
    ...
    const showControls = () => {
      ...
      hideTimer = setTimeout(() => {
        if (!video.paused) document.getElementById('player-overlay')?.classList.remove('visible');
      }, 3000);
    };
...
function selectServer(btn, server) {
  document.querySelectorAll('.filter-chip').forEach(c => c.classList.remove('active'));
  btn.classList.add('active');
  showToast(`Beralih ke ${btn.textContent}`, 'info');
}
...
function setupPlayerGestures() {
  ...
  pw.addEventListener('touchstart', (e) => {
    ...
    // Hold right for 2x speed
    if (isRight) {
      holdTimer = setTimeout(() => {
        isHolding = true;
        prevSpeed = v.playbackRate;
        v.playbackRate = 2;
    ...
    if (now - lastTap < 300) {
      ...
      if (isLeft) playerSkip(-10);
      else if (isRight) playerSkip(10);
    }
    lastTap = now;
  }, {passive: true});
...
function loadAnnouncements() {
  App.announcements = [
    { id: 1, title: 'Selamat Datang di Zentra!', content: 'Platform anime terbaru dengan fitur streaming, wiki, AI assistant, dan banyak lagi.', type: 'Information', priority: 'Normal', active: true, pinned: true, views: 0, createdAt: new Date().toISOString(), createdBy: 'Admin' },
    { id: 2, title: 'Fitur AI Zentra Tersedia', content: 'Tanya rekomendasi anime, penjelasan cerita, atau urutan nonton ke Zentra AI!', type: 'Update', priority: 'High', active: true, pinned: false, views: 0, createdAt: new Date().toISOString(), createdBy: 'Admin' },
    { id: 3, title: 'Server Streaming Normal', content: 'Semua server streaming berjalan normal. Nikmati anime favoritmu!', type: 'System', priority: 'Low', active: true, pinned: false, views: 0, createdAt: new Date().toISOString(), createdBy: 'System' },
  ];
...
function renderAnnouncementGrid(grid) {
  ...
  grid.innerHTML = App.announcements.map(a => `
    <div class="glass-card" style="padding:20px;border-left:3px solid ${typeColor[a.type]||'var(--primary)'}">
      ...
      <div style="font-size:0.72rem;color:var(--muted);margin-top:10px">${new Date(a.createdAt).toLocaleDateString('id-ID')} · Oleh ${a.createdBy}</div>
    </div>`).join('');
}
